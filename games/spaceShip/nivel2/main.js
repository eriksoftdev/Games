...
function mostrarMensajeDisparo(silaba) {
  ...
  if (parrafo) {
    parrafo.textContent = `Dispara a: `;
    /* parrafo.textContent = `Dispara a: ${silaba}`; */

    const botonSonido = document.createElement('button');
    botonSonido.addEventListener('click', () => {
      const audio = new Audio(`audio/audioSilabas/${silaba}.ogg`);
      audio.play();
      
    });

    parrafo.appendChild(botonSonido);
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  function checkCollision(element) {
    ...
    if (
      ...
      if (element.textContent === silabaADisparar && spaceKeyPressed && !espacioPresionado) {
...
let laserSound = new Audio('audio/disparoLaser.ogg'); // Reemplaza 'audio/shot.ogg' con la ruta correcta de tu archivo de sonido
