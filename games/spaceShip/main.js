...
/* //mueve la nave de izquierda a derecha
...
document.addEventListener('keydown', (event) => {
  ...
  if (key === 'ArrowLeft') {
    ...
    currentPosition = Math.min(windowWidth - laserWidth, currentPosition + 20);
    laser.style.left = `${currentPosition}px`;
  }
});
...
function mostrarMensajeDisparo(silaba) {
  ...
  if (divSilabaADisparar && parrafo) {
    divSilabaADisparar.textContent = `${silaba}`;
    parrafo.textContent = `Dispara a: ${silaba}`;
  } else {
    console.error('No se encontró el div con la clase "silabaADisparar" o el párrafo en el footer');
  }
}
